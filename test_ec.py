...
def ec_metrics(output_text, ec_reactions):
...
def main():
    ...
    for i in tqdm(range(len(test_data))):
        ...
        ec_reactions = [ec_terms[ec] for ec in ec_labels if ec in ec_terms]
        if len(ec_reactions) == 0:
            continue
        else:
            best_score_p = 0
            output_file.write(sequence + '\n')
            output_file.write("GT: " + " ".join(ec_reactions) + '\n')
            for p in prompt:
                output_text = generator.generate([p], [sequence])
                
                score = ec_metrics(output_text, ec_reactions)
    ...
    print("Percentage of samples with ROUGE-L score > 0.7: ", np.sum(np.array(all_scores) > 0.7) / len(all_scores))
    print("Percentage of samples with ROUGE-L score > 0.8: ", np.sum(np.array(all_scores) > 0.8) / len(all_scores))
    print("Percentage of samples with ROUGE-L score > 0.9: ", np.sum(np.array(all_scores) > 0.9) / len(all_scores))
    print("Percentage of samples with ROUGE-L score = 1: ", np.sum(np.array(all_scores) == 1.0) / len(all_scores))
